...
import { post, patch, req, type Account, type IncomeCandidate, type IncomeDeduction, type IncomeRecord, type PlanItem } from '../api.js';
...
const STATES = { pending: 'รอเงินเข้าจาก Statement', needs_review: 'ต้องเลือกคู่เงินเข้า', matched: 'ยืนยันจาก Statement แล้ว', not_required: 'ยอดสุทธิเป็นศูนย์' };
type DeductionForm = { deduction_type: IncomeDeduction['deduction_type']; name: string; amount: string; monthly_plan_item_id: string };
type Form = { name: string; gross: string; monthly_plan_item_id: string; bank_account_id: string; income_date: string; auto_match: boolean; deductions: DeductionForm[] };
const emptyForm = (): Form => ({ name: '', gross: '', monthly_plan_item_id: '', bank_account_id: '', income_date: '', auto_match: true, deductions: [] });
...
export default function IncomeSection({ month, closed, items, onChanged }: { month: string; closed: boolean; items: PlanItem[]; onChanged: () => Promise<void> }) {
  ...
  const [matching, setMatching] = useState<IncomeRecord | null>(null);
  const [candidates, setCandidates] = useState<IncomeCandidate[]>([]);
  const [candidateLoading, setCandidateLoading] = useState(false);
  const [unlinking, setUnlinking] = useState<IncomeRecord | null>(null);
  ...
  useEffect(() => {
    ...
    req<{ rows: IncomeCandidate[] }>(`/api/income-records/${matching.id}/candidates`)
      .then((data) => { if (current) setCandidates(data.rows); })
      .catch((e: Error) => { if (current) setFormError(e.message); })
  ...
  const openEditor = (income: IncomeRecord | 'new') => {
    ...
    setForm(income === 'new' ? emptyForm() : {
      ...
      bank_account_id: income.bank_account_id == null ? '' : String(income.bank_account_id), income_date: income.income_date ?? '', auto_match: income.auto_match,
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = { name: form.name, gross_amount_satang: amount(form.gross), bank_account_id: form.bank_account_id ? Number(form.bank_account_id) : null, income_date: form.income_date || null, auto_match: form.auto_match, deductions };
  ...
  const match = async (candidate: IncomeCandidate) => {
    if (!matching) return;
    setBusy(true); setFormError('');
    try { await post(`/api/income-records/${matching.id}/match`, { txn_id: candidate.id }); setMatching(null); await refresh(); }
    catch (e) { setFormError(e instanceof Error ? e.message : 'จับคู่ไม่สำเร็จ'); }
    finally { setBusy(false); }
  };
  ...
  return <Box component="section" aria-labelledby="income-heading">
    <PageHeader id="income-heading" title="รายได้และรายการหัก" description="บันทึกรายได้เต็ม แล้วจับคู่ยอดสุทธิกับเงินเข้าจากธนาคาร" action={<Button startIcon={<AddRounded />} variant="outlined" disabled={closed || loading || Boolean(error)} onClick={() => openEditor('new')}>เพิ่มรายได้เต็ม</Button>} />
    ...
    {loading ? <TableSkeleton rows={2} /> : !error && (rows.length === 0 ? <Typography color="text.secondary" sx={{ mt: 2 }}>ยังไม่มีรายได้เต็มในเดือนนี้ เลือกเชื่อมรายได้เดิมในแผน หรือเพิ่มรายการใหม่ได้</Typography> :
      <TableContainer component={Paper} variant="outlined" tabIndex={0} sx={{ mt: 2 }}>
        <Table size="small" aria-label="รายได้เต็มและเงินเข้าสุทธิ" sx={{ minWidth: 760 }}>
          <TableHead><TableRow><TableCell>รายได้</TableCell><TableCell align="right">รายได้เต็ม</TableCell><TableCell align="right">หักทั้งหมด</TableCell><TableCell align="right">สุทธิ</TableCell><TableCell>เงินเข้า</TableCell><TableCell>จัดการ</TableCell></TableRow></TableHead>
          <TableBody>{rows.map((income) => <TableRow key={income.id}>
            ...
            <TableCell><Chip size="small" label={STATES[income.match_status]} color={income.match_status === 'matched' ? 'success' : 'default'} variant="outlined" />{income.deposit_txn_id != null && <Button component={Link} to={`/transactions?month=${month}&txn=${income.deposit_txn_id}`}>ดูธุรกรรม #{income.deposit_txn_id}</Button>}</TableCell>
            <TableCell><Stack direction="row" spacing={0.5}>
              <Button disabled={closed || income.deposit_txn_id != null} onClick={() => openEditor(income)}>แก้ไข</Button>
              {income.deposit_txn_id != null ? <Button disabled={closed} onClick={() => setUnlinking(income)}>ยกเลิกคู่</Button> : <Button disabled={income.expected_net_satang === 0} onClick={() => setMatching(income)}>เลือกเงินเข้า</Button>}
            </Stack></TableCell>
          </TableRow>)}</TableBody>
        </Table>
      </TableContainer>)}
    <Modal open={editing != null} title={editing === 'new' ? 'เพิ่มรายได้เต็ม' : 'แก้ไขรายได้เต็ม'} onClose={() => setEditing(null)} busy={busy}>
      <Stack component="form" spacing={2} onSubmit={(e) => { e.preventDefault(); void save(); }}>
        ...
        {preview != null && <Typography aria-live="polite">เงินเข้าสุทธิที่คาดไว้: <Money satang={preview} tone={preview < 0 ? 'expense' : 'income'} /></Typography>}
        <TextField select label="บัญชีรับเงิน" value={form.bank_account_id} onChange={(e) => setForm({ ...form, bank_account_id: e.target.value })} required={form.auto_match && preview !== 0}><MenuItem value="">ยังไม่ระบุ</MenuItem>{accounts.map((a) => <MenuItem key={a.id} value={String(a.id)}>{a.nickname}</MenuItem>)}</TextField>
        <TextField label="วันที่คาดรับเงิน" type="date" value={form.income_date} onChange={(e) => setForm({ ...form, income_date: e.target.value })} required={form.auto_match && preview !== 0} slotProps={{ inputLabel: { shrink: true } }} />
        <FormControlLabel control={<Checkbox checked={form.auto_match} onChange={(e) => setForm({ ...form, auto_match: e.target.checked })} />} label="จับคู่เงินเข้าอัตโนมัติ เมื่อบัญชีและยอดตรงกัน วันที่ห่างไม่เกิน 3 วัน" />
        {formError && <Alert severity="error">{formError}</Alert>}
        <Button type="submit" variant="contained" disabled={busy || (preview != null && preview < 0)}>{busy ? 'กำลังบันทึก…' : 'บันทึกรายได้'}</Button>
      </Stack>
    </Modal>
    <Modal open={matching != null} title={`เลือกเงินเข้า — ${matching?.name ?? ''}`} onClose={() => setMatching(null)} busy={busy}>
      <Stack spacing={2}>
        <Typography>เลือก Credit บัญชีเดียวกัน ยอดเท่าสุทธิ และวันที่ห่างไม่เกิน 3 วัน</Typography>
        {candidateLoading ? <TableSkeleton rows={2} /> : candidates.length === 0 && !formError ? <Alert severity="info">ยังไม่มีเงินเข้าที่ตรงกัน ตรวจบัญชีและวันที่คาดรับ หรือรอ Statement รอบถัดไป</Alert> : candidates.map((c) => <Paper key={c.id} variant="outlined" sx={{ p: 2 }}><Stack spacing={1}><Typography>{formatDate(c.txn_date)} · {c.account_nickname}</Typography><Typography sx={{ overflowWrap: 'anywhere' }}>{c.description}</Typography><Money satang={c.amount_satang} /><Button disabled={busy} variant="outlined" onClick={() => void match(c)}>ยืนยันเงินเข้ารายการนี้</Button></Stack></Paper>)}
        {formError && <Alert severity="error">{formError}</Alert>}
      </Stack>
    </Modal>
    <ConfirmDialog open={unlinking != null} title="ยกเลิกคู่เงินเข้า" description="รายได้จะกลับไปรอเงินเข้า และปิดการจับคู่อัตโนมัติ คุณเปิดใหม่ได้จากแก้ไขรายได้" confirmLabel="ยกเลิกคู่" busy={busy} onClose={() => setUnlinking(null)} onConfirm={() => {
      if (!unlinking) return;
      setBusy(true);
      void post(`/api/income-records/${unlinking.id}/unmatch`, {}).then(async () => { setUnlinking(null); await refresh(); }).catch((e: Error) => { setUnlinking(null); setError(e.message); }).finally(() => setBusy(false));
    }} />
  </Box>;
}
